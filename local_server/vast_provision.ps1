...
function Set-AndVerifyVastKey([string]$Key) {
    ...
    $script:CurrentUser = Get-VastUser
...
function Test-AccountCredit {
    if (-not $script:CurrentUser) { $script:CurrentUser = Get-VastUser }
    if (-not $script:CurrentUser) { return $false }

    $email = Get-PropertyValue $script:CurrentUser @("email", "normalized_email", "username")
    $userId = Get-PropertyValue $script:CurrentUser @("id", "user_id")
    $teamName = Get-PropertyValue $script:CurrentUser @("team_name")
    $teamId = Get-PropertyValue $script:CurrentUser @("team_id")

    $identity = if ($email) { [string]$email } else { "unknown email" }
    if ($userId) { $identity += " (user ID $userId)" }
    if ($teamName) { $identity += " | team: $teamName" }
    elseif ($teamId) { $identity += " | team ID: $teamId" }
    Write-Step "Authenticated Vast.ai account: $identity"

    $rawCredit = Get-PropertyValue $script:CurrentUser @("credit", "balance")
    if ($null -eq $rawCredit) {
        Write-Warn "Vast.ai did not return a credit/balance field; continuing without a local balance block."
        return $true
    }

    try { $credit = [double]$rawCredit }
    catch {
        Write-Warn "Could not parse Vast.ai account credit '$rawCredit'; continuing without a local balance block."
        return $true
    }

    Write-Host ("  Vast.ai account credit: `${0:N2}" -f $credit) -ForegroundColor Cyan
    if ($credit -le 0) {
        Write-Err "This authenticated Vast.ai account currently reports no usable credit."
        Write-Warn "Confirm the email/user ID above matches the browser account where you purchased credits."
        Write-Warn "If you purchased credits for a team, create/use a team API key or transfer credit to this account context."
        Write-Warn "Vast says balances normally update within a few seconds; refresh Billing and retry after the payment is completed."
        return $false
    }
...
function Get-OfferPrice($Offer) {
    $value = Get-PropertyValue $Offer @("dph_total", "dph_base", "dph", "min_bid")
...
function Search-Offers {
    ...
    try {
        ...
        for ($i = 0; $i -lt $offers.Count; $i++) {
            $offer = $offers[$i]
            $price = Get-OfferPrice $offer
            $priceText = if ($null -ne $price) { ('${0:N3}/hr' -f $price) } else { "price unavailable" }
            Write-Host ("  [{0}] ID {1} | {2} | {3}" -f ($i + 1), $offer.id, $offer.gpu_name, $priceText)
        }
...
function Create-Instance([string]$OfferId) {
    ...
    try {
        ...
        if ($message -match 'lacks credit|insufficient.*credit|balance') {
            Write-Err "Vast.ai rejected the rental because this API-key account has insufficient usable credit."
            Write-Warn "Compare the authenticated account shown above with the account/team that received your payment."
        } else {
            Write-Err $message
        }
        return $null
...
function Wait-ForHealth([string]$Url) {
    ...
    Write-Step "The first launch may take 30-60 minutes while CUDA/PyTorch packages download."
    $started = Get-Date
    $deadline = $started.AddMinutes(75)
    $nextProgress = $started.AddMinutes(1)
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        if ((Get-Date) -ge $nextProgress) {
            ...
            Write-Host ""
            Write-Step "Still installing after $elapsed minute(s); the instance remains active and billing."
            $nextProgress = (Get-Date).AddMinutes(1)
        } else {
            Write-Host "." -NoNewline -ForegroundColor DarkGray
        }
        Start-Sleep 10
    ...
    Write-Host ""
    Write-Warn "The server did not become healthy within 75 minutes."
    Write-Warn "The instance is still active and billing; inspect /var/log/grok-video-studio-bootstrap.log."
    return $false
}
...
$healthy = Wait-ForHealth $url
if (-not $healthy) {
    Write-Warn "Provisioning is incomplete. Stop the instance or inspect the bootstrap log before continuing."
    exit 2
}
