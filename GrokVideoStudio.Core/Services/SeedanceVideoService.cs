...
/// <summary>
/// Seedance 2.0 Video Generation API implementation.
/// Uses POST to https://api.seedance.ai/v2/generations with model 'seedance-2.0',
/// and polls GET /v2/generations/{id} until terminal state.
/// </summary>
public sealed class SeedanceVideoService(HttpClient httpClient) : IVideoGenerationService
{
    private const string BaseUrl = "https://api.seedance.ai";
    private const string GenerationsEndpoint = "/v2/generations";
    private const string VideoStatusEndpoint = "/v2/generations/";

    /// <inheritdoc />
    public VideoProvider Provider => VideoProvider.Seedance;
        ...
        CancellationToken ct = default)
    {
        ...
        progress?.Report("Submitting to Seedance 2.0 API…");
        ...
        if (!submitResp.IsSuccessStatusCode)
            throw new HttpRequestException($"Seedance submit failed: {submitResp.StatusCode} — {TryExtractError(submitJson)}");
        ...
        if (string.IsNullOrEmpty(id))
            throw new InvalidOperationException("Seedance returned a response without a generation ID.");
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++)
        {
            ...
            progress?.Report($"Polling Seedance {attempt}/{maxAttempts}…");
